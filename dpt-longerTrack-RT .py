...
class DrunkDetectionReID:
    ...
    def process_frame(self, frame):
        ...
        Ensures correct bounding boxes & tracking IDs per video.
        """
        ...
        for track in tracked_objects:
            if track.is_confirmed():
                ...
                state_color = (0, 255, 0)  # Default: Normal
                label = f"Person {track_id}: Normal"

                # Ensure the confidence value is always valid
                confidence = track.det_conf if track.det_conf is not None else 0.0

                # Store person data to ensure long-term Re-ID
                if track_id not in self.track_memory or confidence > 0.7:
                    self.track_memory[track_id] = track.features
                ...
                # Always show bounding boxes
                cv2.putText(frame, label, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, state_color, 2)
                cv2.rectangle(frame, (x1, y1), (x2, y2), state_color, 2)

        return frame

# FOR REALTIME VIDEO PROCESSING - Change .yaml to accomodate ip camera. i.e. `video_path1: "rtsp://username:password@192.168.1.100:554/stream1"`
def process_video(video_path, window_name, drunk_detector): 
    cap = cv2.VideoCapture(video_path)
    
    if not cap.isOpened():
        logger.error(f"Failed to open stream: {video_path}")
        return
    ...
    while True:  # Keeps running even if frames are dropped
        ret, frame = cap.read()
        
        if not ret:
            logger.warning(f"Lost connection to {video_path}, retrying...")
            time.sleep(2)  # Wait 2 seconds before retrying
            cap = cv2.VideoCapture(video_path)  # Reconnect to stream
        ...
        if cv2.waitKey(1) & 0xFF == ord('q'):
            break  # Allows user to quit with 'q'
